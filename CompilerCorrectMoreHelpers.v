...
From DanTrick Require Import ProofSubstitution.
Lemma var_map_wf_wrt_bexp_subset_stronger :
  forall (b: bexp_Dan) (idents idents': list ident),
    (forall x,
        In x idents ->
        In x idents') ->
    NoDup idents' ->
...
Proof.
  induction b; intros.
  - apply true_dan_always_well_formed. assumption.
  - apply false_dan_always_well_formed. assumption.
  - eapply var_map_wf_neg_dan_forwards. reflexivity.
    eapply var_map_wf_neg_dan in H2; [ | reflexivity ].
    eapply IHb; eassumption.
  - eapply var_map_wf_and_or_dan_backwards. left. reflexivity. eapply var_map_wf_and_or_dan_forwards in H2; [ | left; reflexivity ].
    destruct H2. split.
    + eapply IHb1; eassumption.
    + eapply IHb2; eassumption.
  - eapply var_map_wf_and_or_dan_backwards. right. reflexivity. eapply var_map_wf_and_or_dan_forwards in H2; [ | right; reflexivity ].
    destruct H2. split.
    + eapply IHb1; eassumption.
    + eapply IHb2; eassumption.
  - eapply var_map_wf_leq_dan_backwards. reflexivity. eapply var_map_wf_leq_dan_forwards in H2; [ | reflexivity ]. destruct H2.
...
Lemma var_map_wf_wrt_imp_subset' :
  forall (i: imp_Dan) (idents: list ident),
...
Lemma var_map_wf_imp_self :
  forall (idents: list ident) (i: imp_Dan),
  ...
  - specialize (nodup_construct_trans i). intros. revert ID. revert idents. induction i; intros.
    ...
    + pose proof (ID' := ID).
      ...
      * revert ID'. subst. intros. specialize (IHi (nodup_construct_trans i) (construct_trans i) eq_refl).
        apply var_map_wf_wrt_imp_subset' with (idents' := (construct_trans (WHILE_Dan b i))) in IHi; try assumption.
