...
From Imp_LangTrick Require Import Base Imp_LangTrickLanguage Imp_LangLogProp StackLanguage StackLogicGrammar LogicProp Imp_LangLogSubst StackLogicBase LogicTranslationCompilerAgnostic ImpVarMap FunctionWellFormed ParamsWellFormed StackLogic Imp_LangLogHoare.
From Imp_LangTrick Require Import Imp_LangTrickLanguage Imp_LangLogProp StackLanguage StackLogicGrammar LogicProp EnvToStackIncomplete ProofCompilableCodeCompiler Imp_LangLogHoare StackUpdateAdequacy TranslationPure ProofCompCodeCompAgnosticMod.

Module IncompleteCodeCompiler <: CheckableCodeCompilerType.
  Definition compile_aexp (exp: aexp_Imp_Lang) (idents: list ident) (num_locals: nat): aexp_stack :=
    compile_aexp_incomplete exp (ident_list_to_map idents) num_locals.

  Definition compile_bexp (exp: bexp_Imp_Lang) (idents: list ident) (num_locals: nat): bexp_stack :=
    compile_bexp_incomplete exp (ident_list_to_map idents) num_locals.

  Definition compile_imp (imp: imp_Imp_Lang) (idents: list ident) (num_locals: nat) :=
    compile_imp_incomplete imp (ident_list_to_map idents) num_locals.

  Definition idents_to_map := ident_list_to_map.

  Definition check_aexp (_: aexp_Imp_Lang) := true.

  Definition check_bexp (b: bexp_Imp_Lang) :=
    match b with
    | TRUE_Imp_Lang => true
    | FALSE_Imp_Lang => true
    | _ => false
  ...
  Definition check_imp (i: imp_Imp_Lang) :=
    match i with
    | SKIP_Imp_Lang => true
    | ASSIGN_Imp_Lang x a => true
    | _ => false
    end.
End IncompleteCodeCompiler.

Module IncompleteSpecCompiler <: CheckableSpecificationCompilerType.
  (* Include IncompleteCodeCompiler. *)
  Include CreateStandardCheckableLogicCompiler(IncompleteCodeCompiler) (BasicSpecificationChecker).
  (* Module CC := IncompleteCodeCompiler. *)
...
End IncompleteSpecCompiler.

Require Import Coq.Init.Logic.

Import EqNotations.

Module IncompleteProofCompilable <: ProofCompilableType.
  Module CC := IncompleteCodeCompiler.
  Module SC := IncompleteSpecCompiler.
  ...
  Proof.
    intros. unfold CC.compile_imp. unfold compile_imp_incomplete. reflexivity.
  Qed.
  ...
  Proof.
    intros. unfold CC.compile_imp. unfold compile_imp_incomplete. unfold CC.compile_aexp. reflexivity.
  Qed.
...
End IncompleteProofCompilable.

Require Import ProofCompilerCodeCompAgnostic.

Module IncompleteProofCompiler := CompilerAgnosticProofCompiler(IncompleteProofCompilable).
