...
Require Import DanTrick.StackFrame1 DanTrick.StackFrameMinHelpers.

From DanTrick Require Import StackFrameZTheorems.
...
Proof.
  ...
  - destruct H. destruct H.
    ...
    + intros. invs H1.
      ...
      lia.
  - exists 0. split.
    + simpl. rewrite minus_plus. constructor.
    + intros. lia.
  ...
  Proof.
    ...
    eapply stack_mutated_at_index_preserved_by_superlist with (other := stk2++ stk1) in s. econstructor.
      ...
      + repeat rewrite app_length. rewrite Nat.add_assoc.
        repeat eapply le_plus_trans. assumption.
    ...
    - invs H4.
      ...
      subst stk'. econstructor.
      + eapply H; try eassumption; try reflexivity.
      + eapply H0; try eassumption; try reflexivity.
    - invs H4. replace stk' with (vals ++ stk1) in * by (eapply bexp_frame_implies_pure; eassumption). eapply Stack_if_false.
      + eapply H; try eassumption; try reflexivity.
      + eapply H0; try eassumption; try reflexivity.
    ...
    - subst stk stk'. invc H4. invc H6.
      ...
      + rewrite app_assoc in i.
        ...
        eapply H0.
        reflexivity.
        rewrite app_assoc in Hstk2. eassumption.
        reflexivity.
        destruct H8.
        rewrite e.
        rewrite app_length.
      ...
      + eapply H1.
        * rewrite app_assoc in Hstk2. assumption.
        * rewrite app_assoc in Hstk2. eassumption.
        * reflexivity.
        * reflexivity.
...
Proof.
  ...
  - invs H1. eapply H. invs H0.
    ...
    assumption.
  - invs H2. invs H1.
    eapply H0 with (stk'1 := stk'0) in H9.
    eapply H with (stk'0 := stk') in H6.
    destruct_exists.
    destruct_exists.
    rewrite <- A0 in H6.
    ...
    assumption.
    eauto.
    apply H with (stk'0 := stk') (v := b1) in H6.
    destruct_exists.
  ...
  - invs H1.  invs H2.
    ...
    assumption.
    eauto.
    apply H with (stk'0 := stk') (v := n1) in H6.
    destruct_exists.
    rewrite <- H6. rewrite app_length.
    rewrite A. lia.
    reflexivity.
  ...
  - invs H.
    ...
    apply firstn_length_le. assumption.
  - invs H1. invs H2.
    apply H with (stk'0 := stk') (v := val) in H6; apply H0 with (stk'1 := stk'0) (vals0 := vals) in H9.
    ...
    reflexivity.
    reflexivity.
    apply H with (stk'0 := stk') (v := val) in H6. destruct_exists.
    rewrite <- H6. rewrite A0. assumption.
    reflexivity.
    assumption.
    assumption.
...
Proof.
  ...
  - rewrite H1 in H0.
    ...
    econstructor.
  - invs H0.
    simplify_stacks. eapply H with (stk2 := stk1) (x0 := x) (v := c) (stk' := stk1 ++ x) (x' := x') in H12. apply stack_mutated_at_index_app with (x' := x') in s.
    econstructor.
