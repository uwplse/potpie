...
From DanTrick Require Import ProofCompiler StackLangTheorems ImpVarMap DanImpHigherOrderRel ProofCompilerBase DanLogProp LogicProp ProofCompilerHelpers DanHoareWF DanLogHoare DanImpHigherOrderRelTheorems LogicTranslationBase LogicTranslationAdequate DanTrickLanguage.
...
Lemma containment_disjunction :
  forall (x: DanTrickLanguage.ident) (setA setB: ListSet.set ident) (idents: list ident),
    NoDup idents ->
    NoDup setA ->
    NoDup setB ->
    In x idents ->
...
Lemma free_vars_in_imp_alternate :
  forall (idents: list ident) (i: imp_Dan) (x: ident),
    In x idents ->
...
Lemma second_wf_proof :
  forall (idents: list string)
    (index : nat) (x : StackLanguage.ident),
...
Lemma well_formed_assign_param1 :
  imp_rec_rel (var_map_wf_wrt_imp ("z" :: nil)) ("z" <- PARAM_Dan 1).
Proof.
  pose proof (H := "z" = "z").
